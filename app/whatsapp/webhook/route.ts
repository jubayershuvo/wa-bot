...
const INSTANT_SERVICES = [
  ...
  {
    id: "instant_holding_payment_link",
    name: "💰 Make Holding Payment Link",
    description: "হোল্ডিং পেমেন্ট লিংক তৈরি করুন",
    price: 15,
    isActive: true,
    requiresInput: true,
    inputPrompt: "Dakhila URL টি পাঠান:",
...
// Add this function after other service handler functions (around line 1350)

async function handleDakhilaApprovalCheck(
  phone: string,
  dakhilaUrl: string,
): Promise<void> {
  ...
  EnhancedLogger.info(
    `Processing Dakhila approval check for ${formattedPhone}`,
    {
  ...
  try {
    ...
    const service = INSTANT_SERVICES.find(
      (s) => s.id === "instant_dakhila_approval",
    );
    if (!service) {
      await sendTextMessage(
        formattedPhone,
        "❌ Dakhila service not available!",
      );
      await showMainMenu(formattedPhone, false);
      return;
    }
    ...
    if (result.status === "success") {
      ...
      resultMessage += `❌ *Dakhila Approval Status:* **NOT APPROVED**\n\n`;
      resultMessage += `Dakhila URL টি যাচাই করা হয়েছে এবং এটি কোন অনুমোদন পাওয়া যায়নি।\n`;
    } else {
      resultMessage += `❓ *Dakhila Approval Status:* **UNKNOWN**\n\n`;
      resultMessage += `Dakhila URL টি যাচাই করা হয়েছে কিন্তু স্ট্যাটাস নির্ধারণ করা যায়নি।\n`;
    }

    if (result.message) {
      resultMessage += `\n📝 *বিস্তারিত:* ${result.message}\n`;
    }
    ...
    // Notify admin
    await notifyAdmin(
      `✅ Dakhila Approval Check সম্পন্ন\n\nব্যবহারকারী: ${formattedPhone}\nসার্ভিস: ${service.name}\nমূল্য: ৳${service.price}\nURL: ${dakhilaUrl.substring(0, 100)}${dakhilaUrl.length > 100 ? "..." : ""}\nস্ট্যাটাস: ${result.status === "success" ? "✅ APPROVED" : result.status === "error" ? "❌ NOT APPROVED" : "❓ UNKNOWN"}\nব্যালেন্স: ${oldBalance} → ${user.balance}`,
    );

    await stateManager.clearUserState(formattedPhone);
    await showMainMenu(formattedPhone, false);

    EnhancedLogger.logFlowCompletion(formattedPhone, "dakhila_approval", {
      url: dakhilaUrl,
      price: service.price,
    ...
    EnhancedLogger.error(`Failed to process Dakhila approval:`, {
    ...
    await sendTextMessage(
      formattedPhone,
      "❌ Dakhila approval যাচাই করতে সমস্যা হয়েছে। দয়া পরে চেষ্টা করুন।\n\n🏠 মেনুতে ফিরে যেতে 'Menu' লিখুন",
    );
...
// Add this function after handleDakhilaApprovalCheck function
async function handleHoldingPaymentLink(
  phone: string,
  dakhilaUrl: string,
): Promise<void> {
  const formattedPhone = formatPhoneNumber(phone);

  EnhancedLogger.info(`Processing Holding Payment Link for ${formattedPhone}`, {
    urlPreview: dakhilaUrl.substring(0, 100),
  });
  ...
  try {
    ...
    const service = INSTANT_SERVICES.find(
      (s) => s.id === "instant_holding_payment_link",
    );
    if (!service) {
      await sendTextMessage(
        formattedPhone,
        "❌ Payment Link service not available!",
      );
      await showMainMenu(formattedPhone, false);
      return;
    }
    ...
    await sendTextMessage(
      formattedPhone,
      `⏳ *হোল্ডিং পেমেন্ট লিংক তৈরি করা হচ্ছে...*\n\nURL: ${dakhilaUrl.substring(0, 80)}${dakhilaUrl.length > 80 ? "..." : ""}\n\nদয়া করে অপেক্ষা করুন...`,
    );
    ...
    if (result.status === "success" && result.paymentUrl) {
      ...
      if (result.totalDemand) {
        resultMessage += `💰 *মোট পেমেন্ট ধার্য:* ৳${result.totalDemand}\n\n`;
      }

      resultMessage += `⚠️ *গুরুত্বপূর্ণ সতর্কতা:*\n`;
      resultMessage += `• এই লিংকটি *একবার মাত্র* ভিজিট করা যাবে\n`;
      resultMessage += `• একবার ভিজিট করলে লিংকটি বন্ধ হয়ে যাবে\n`;
      resultMessage += `• লিংকটি কাউকে শেয়ার করবেন না\n`;
      resultMessage += `• সরাসরি পেমেন্ট করার জন্য লিংকটি ওপেন করুন\n`;
      resultMessage += `• পেমেন্ট সম্পূর্ণ না হওয়া পর্যন্ত ব্রাউজার বন্ধ করবেন না\n\n`;

      resultMessage += `📝 *পেমেন্ট নির্দেশনা:*\n`;
      resultMessage += `1. উপরের লিংকটিতে ক্লিক করুন\n`;
      resultMessage += `2. পেমেন্ট পেজে আপনার তথ্য যাচাই করুন\n`;
      resultMessage += `3. পেমেন্ট মাধ্যম নির্বাচন করুন\n`;
      resultMessage += `4. পেমেন্ট সম্পন্ন করুন\n`;
      resultMessage += `5. কনফার্মেশন রসিদ সংরক্ষণ করুন\n`;
    } else if (result.status === "error") {
      resultMessage += `❌ *পেমেন্ট লিংক তৈরি ব্যর্থ*\n\n`;
      resultMessage += `কারণ: ${result.message || "অজানা সমস্যা"}\n\n`;
      resultMessage += `দয়া করে সঠিক Dakhila URL দিন অথবা পরে চেষ্টা করুন।\n`;
    } else {
      resultMessage += `❓ *পেমেন্ট লিংক তৈরি সম্ভব হয়নি*\n\n`;
    ...
    // Notify admin
    await notifyAdmin(
      `✅ Holding Payment Link তৈরি সম্পন্ন\n\nব্যবহারকারী: ${formattedPhone}\nসার্ভিস: ${service.name}\nমূল্য: ৳${service.price}\nURL: ${dakhilaUrl.substring(0, 100)}${dakhilaUrl.length > 100 ? "..." : ""}\nস্ট্যাটাস: ${result.status === "success" ? "✅ SUCCESS" : "❌ FAILED"}\nপেমেন্ট লিংক: ${result.paymentUrl ? "✅ তৈরি করা হয়েছে" : "❌ তৈরি করা যায়নি"}\nমোট ধার্য: ${result.totalDemand ? `৳${result.totalDemand}` : "N/A"}\nব্যালেন্স: ${oldBalance} → ${user.balance}`,
    );

    await stateManager.clearUserState(formattedPhone);
    await showMainMenu(formattedPhone, false);

    EnhancedLogger.logFlowCompletion(formattedPhone, "holding_payment_link", {
      url: dakhilaUrl,
      price: service.price,
      result: result.status,
      paymentUrl: result.paymentUrl,
    ...
    EnhancedLogger.error(`Failed to generate payment link:`, {
      error: err?.message || err,
      stack: err?.stack,
      phone: formattedPhone,
    });
    ...
    await sendTextMessage(
      formattedPhone,
      "❌ পেমেন্ট লিংক তৈরি করতে সমস্যা হয়েছে। দয়া পরে চেষ্টা করুন।\n\n🏠 মেনুতে ফিরে যেতে 'Menu' লিখুন",
    );
...
async function checkDakhilaApproval(
  ...
  try {
    EnhancedLogger.info(`Checking Dakhila approval for URL`, {
    ...
    EnhancedLogger.debug(`Calling Dakhila API`, {
    ...
    if (!response.ok) {
      EnhancedLogger.error(`Dakhila API error: ${response.status}`, {
        status: response.status,
        statusText: response.statusText,
        responseTime: `${responseTime}ms`,
      });
      ...
      if (response.status === 404) {
        errorMessage = "Dakhila API endpoint not found";
      } else if (response.status === 400) {
        errorMessage = "Invalid request to Dakhila API";
      } else if (response.status === 429) {
        errorMessage = "Too many requests to Dakhila API";
      } else if (response.status >= 500) {
        errorMessage = "Dakhila API server error";
      }
    ...
    EnhancedLogger.debug(`Dakhila API response`, {
      status: data?.status,
      responseTime: `${responseTime}ms`,
      hasData: !!data,
    ...
    if (data && data.status === "success") {
      return {
        status: "success",
        message: data.message || "Dakhila has approval",
        data: data.data || data,
      };
    } else if (data && data.status === "error") {
      return {
        status: "error",
        message: data.message || "Dakhila hasn't approval",
        data: data,
      };
    } else if (data && data.approved !== undefined) {
      // Alternative response format
      return {
        status: data.approved ? "success" : "error",
        message: data.message || (data.approved ? "Approved" : "Not approved"),
        data: data,
      };
    } else {
      EnhancedLogger.warn(`Unexpected Dakhila API response`, {
        data,
      });
      return {
        status: "unknown",
        message: "Could not determine approval status from response",
        data: data,
      };
    }
  } catch (error: any) {
    EnhancedLogger.error(`Error checking Dakhila approval:`, {
      error: error?.message || error,
      stack: error?.stack,
      url: url.substring(0, 100),
    });

    let errorMessage = "Failed to check Dakhila approval";
    if (error.message?.includes("timeout")) {
      errorMessage = "Dakhila API timeout. Please try again later.";
    } else if (error.message?.includes("network")) {
      errorMessage = "Network error. Please check your connection.";
    } else if (error.message?.includes("ENOTFOUND")) {
      errorMessage = "Cannot connect to Dakhila API server.";
...
// Add this function after checkDakhilaApproval function
async function generatePaymentLink(url: string): Promise<{
  status: string;
  message?: string;
  paymentUrl?: string;
  totalDemand?: number;
  ...
  try {
    EnhancedLogger.info(`Generating payment link for URL`, {
      urlLength: url.length,
      urlPreview: url.substring(0, 100),
    });
    ...
    const encodedUrl = encodeURIComponent(processedUrl);
    const apiUrl = `https://api.fortest.top/dakhila/payment_link_gen.php?url=${encodedUrl}`;

    EnhancedLogger.debug(`Calling Payment Link API`, {
      apiUrl: apiUrl.substring(0, 200) + "...",
    });
    ...
    if (!response.ok) {
      EnhancedLogger.error(`Payment Link API error: ${response.status}`, {
        status: response.status,
        statusText: response.statusText,
        responseTime: `${responseTime}ms`,
      });
      ...
      let errorMessage = `API request failed with status ${response.status}`;
      if (response.status === 404) {
        errorMessage = "Payment Link API endpoint not found";
      } else if (response.status === 400) {
        errorMessage = "Invalid request to Payment Link API";
      } else if (response.status === 429) {
        errorMessage = "Too many requests to Payment Link API";
      } else if (response.status >= 500) {
        errorMessage = "Payment Link API server error";
      }
    ...
    EnhancedLogger.debug(`Payment Link API response`, {
      status: data?.status,
      responseTime: `${responseTime}ms`,
      hasData: !!data,
      dataKeys: data ? Object.keys(data) : [],
    ...
    if (data && data.url) {
      return {
        ...
        paymentUrl: data.url,
        totalDemand: data.holding_data?.total_demand || data.total_demand,
      };
    } else if (data && data.error) {
      return {
        status: "error",
        message: data.error || "Failed to generate payment link",
        paymentUrl: data.url,
        totalDemand: data.holding_data?.total_demand || data.total_demand,
      };
    } else {
      EnhancedLogger.warn(`Unexpected Payment Link API response`, {
        data,
      });
      return {
        status: "error",
        message: "Could not generate payment link from response",
        paymentUrl: data.url,
        totalDemand: data.holding_data?.total_demand || data.total_demand,
      };
    }
  } catch (error: any) {
    EnhancedLogger.error(`Error generating payment link:`, {
      error: error?.message || error,
      stack: error?.stack,
      url: url.substring(0, 100),
    });
    ...
    if (error.message?.includes("timeout")) {
      errorMessage = "Payment Link API timeout. Please try again later.";
    } else if (error.message?.includes("network")) {
      errorMessage = "Network error. Please check your connection.";
    } else if (error.message?.includes("ENOTFOUND")) {
      errorMessage = "Cannot connect to Payment Link API server.";
    } else if (error.message) {
      errorMessage = `Error: ${error.message}`;
    }
...
// Add this function after handleUbrnVerificationStart function (around line 1000)
async function handleDakhilaApprovalStart(phone: string): Promise<void> {
  const formattedPhone = formatPhoneNumber(phone);
  EnhancedLogger.info(`Starting Dakhila approval check for ${formattedPhone}`);
  ...
  try {
    ...
    const message = `✅ *Dakhila Approval Check*\n\n💰 মূল্য: ৳${service.price}\n\n${service.inputPrompt}\n\nউদাহরণ: ${service.inputExample}\n\n📌 নোট:\n• Dakhila URLটি সঠিকভাবে লিখুন\n• যাচাই করতে ১-২ মিনিট সময় লাগতে পারে\n• শুধুমাত্র ldtax.gov.bd এর URL সমর্থিত\n\n🚫 বাতিল করতে নিচের বাটন ক্লিক করুন`;

    await sendTextWithCancelButton(formattedPhone, message);
    EnhancedLogger.info(`Dakhila approval started for ${formattedPhone}`);
  } catch (err) {
    EnhancedLogger.error(`Failed to start Dakhila approval for ${phone}:`, err);
    await sendTextMessage(
      formattedPhone,
      "❌ Dakhila সার্ভিস শুরু করতে সমস্যা হয়েছে। দয়া পরে চেষ্টা করুন।",
    );
    await showMainMenu(formattedPhone, false);
  }
}
// Add this function after handleDakhilaApprovalStart function
async function handleHoldingPaymentLinkStart(phone: string): Promise<void> {
  const formattedPhone = formatPhoneNumber(phone);
  EnhancedLogger.info(`Starting Holding Payment Link for ${formattedPhone}`);
  ...
  try {
    ...
    await stateManager.setUserState(formattedPhone, {
      currentState: "awaiting_payment_link_url",
      flowType: "holding_payment_link",
      data: {
        paymentLinkData: {
          serviceId: "instant_holding_payment_link",
          price: service.price,
    ...
    const message = `💰 *হোল্ডিং পেমেন্ট লিংক তৈরি করুন*\n\n💰 মূল্য: ৳${service.price}\n\n${service.inputPrompt}\n\nউদাহরণ: ${service.inputExample}\n\n📌 নোট:\n• Dakhila URLটি সঠিকভাবে লিখুন\n• লিংক তৈরি করতে ১-২ মিনিট সময় লাগতে পারে\n• শুধুমাত্র ldtax.gov.bd এর URL সমর্থিত\n\n🚫 বাতিল করতে নিচের বাটন ক্লিক করুন`;

    await sendTextWithCancelButton(formattedPhone, message);
    EnhancedLogger.info(`Holding Payment Link started for ${formattedPhone}`);
  } catch (err) {
    EnhancedLogger.error(`Failed to start Payment Link for ${phone}:`, err);
    await sendTextMessage(
      formattedPhone,
      "❌ পেমেন্ট লিংক সার্ভিস শুরু করতে সমস্যা হয়েছে। দয়া পরে চেষ্টা করুন।",
    );
    await showMainMenu(formattedPhone, false);
  }
}
...
function validateDakhilaUrl(url: string): { isValid: boolean; error?: string } {
  try {
    ...
    // Check if it's from ldtax.gov.bd
    if (!urlObj.hostname.includes("ldtax.gov.bd")) {
      return {
        isValid: false,
        error: "URL must be from ldtax.gov.bd domain",
      };
    }

    // Check if it's a citizen/holding URL
    if (!urlObj.pathname.includes("/citizen/holding/")) {
      return {
        isValid: false,
        error: "Invalid Dakhila URL format. Must be a citizen holding URL.",
      };
    }
...
async function triggerAutoFileDelivery(
  ...
  EnhancedLogger.info(
    `[${requestId}] Starting auto file delivery for admin: ${formattedPhone}`,
    {
      messageType: message.type,
      hasImage: !!message.image,
      hasDocument: !!message.document,
  ...
  try {
    ...
    if (message.type === "image" || message.type === "document") {
      ...
      try {
        EnhancedLogger.info(
          `[${requestId}] Downloading WhatsApp media: ${mediaId}`,
        );
      ...
      const individualWords = cleanFileName
        .split(/\s+/)
      ...
      const autoDeliveryBaseDir = path.join(
        process.cwd(),
        "uploads",
        "auto_delivery",
      );
      ...
      try {
        if (!fs.existsSync(deliveryDir)) {
          ...
          EnhancedLogger.info(
            `[${requestId}] Created delivery directory: ${deliveryDir}`,
          );
        }
      } catch (dirError: any) {
      ...
      try {
        ...
        const stats = fs.statSync(filePath);
        EnhancedLogger.info(
          `[${requestId}] File saved to server successfully`,
          {
            filePath,
            fileSize: stats.size,
      ...
      const relativePath = path
        .join("auto_delivery", year, month, day, uniqueFileName)
        .replace(/\\/g, "/");
      ...
      try {
        ...
        EnhancedLogger.info(
          `[${requestId}] Found ${orders.length} pending orders`,
        );
      } catch (dbError: any) {
      ...
      for (const order of orders) {
        try {
          ...
          if (!user || !user.whatsapp) {
            EnhancedLogger.debug(
              `[${requestId}] Skipping order ${order._id} - no user or whatsapp`,
            );
            continue;
          }
          ...
          if (deliveredUsers.has(user._id.toString())) {
            EnhancedLogger.debug(
              `[${requestId}] Skipping duplicate user ${user._id}`,
            );
            continue;
          }
            ...
            for (const field of order.serviceData) {
              if (field.type === "text" && field.data) {
                try {
                  ...
                  EnhancedLogger.warn(
                    `[${requestId}] Error processing field:`,
                    {
                      orderId: order._id,
      ...
      if (matchedOrders.length === 0) {
        ...
        const noMatchMessage =
          `❌ *কোন ম্যাচিং অর্ডার পাওয়া যায়নি*\n\n` +
      ...
      EnhancedLogger.info(
        `[${requestId}] Starting delivery to ${matchedOrders.length} matched orders`,
        {
      ...
      for (const order of matchedOrders) {
        try {
          ...
          try {
            ...
            EnhancedLogger.info(
              `[${requestId}] WhatsApp template sent successfully`,
              {
            ...
            EnhancedLogger.warn(
              `[${requestId}] WhatsApp template failed, using text fallback`,
              {
            ...
            try {
              ...
              EnhancedLogger.info(
                `[${requestId}] Text fallback sent successfully`,
                {
                  userPhone: order.userPhone,
                },
              );
              ...
              EnhancedLogger.error(
                `[${requestId}] Text fallback also failed:`,
                {
      ...
      try {
        ...
        EnhancedLogger.error(
          `[${requestId}] Failed to send admin notification:`,
          {
      ...
      EnhancedLogger.warn(
        `[${requestId}] Unsupported message type for auto delivery`,
        {
          messageType: message.type,
        },
      );
...
async function handleUserMessage(
  ...
  try {
    ...
    if (isAdmin && (message.type === "image" || message.type === "document")) {
      ...
      if (isAtMainMenu) {
        EnhancedLogger.info(
          `[${requestId}] Admin at main menu uploaded file, starting auto delivery`,
          {
            currentState,
            flowType,
            messageType: message.type,
        ...
        EnhancedLogger.info(
          `[${requestId}] Admin uploaded file but is in active flow, NOT triggering auto delivery`,
          {
            currentState,
            flowType,
            messageType: message.type,
